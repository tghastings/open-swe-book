...
- 📄 **SEC Release No. 34-70694 — In the Matter of Knight Capital Americas LLC** (October
  ...
  enforcement order behind §14.3.5's first case study: the reused flag, the eighth server,
...
- 📄 **Kubernetes documentation** — [kubernetes.io/docs](https://kubernetes.io/docs/).
  The official concepts, tutorials, and reference for §14.1.5: containers, pods, services,
...
- 📘 **37signals — "We have left the cloud"** —
  ...
  first-person record of §14.1.4's repatriation case study; the announcement post
...
- 📘 **InfoQ — "Prime Video Switched from Serverless to EC2 and ECS to Save Costs"** (May
  ...
  monolith-style process (§14.1.4). The original Prime Video Tech post (Marcin Kolny,
...
- 📄 **Gilbert & Lynch — "Brewer's Conjecture and the Feasibility of Consistent,
  ...
  turned Brewer's conjecture into §14.1.6's CAP theorem by proving it; author copies
...
- 📄 **AWS — Shared Responsibility Model, and "What's the Difference Between Containers
  ...
  Vendor statements of §14.1.4's security-responsibility split ("security *of* the cloud"
  versus "security *in* the cloud") and §14.1.5's container/VM distinction. *Access: free
  to read.*
- 📄 **Synergy Research Group — cloud market share updates** —
  ...
  Quarterly market data behind §14.1.4's "big three" claim: Amazon, Microsoft, and Google
...
- 📄 **CNCF Annual Survey 2023** —
  ...
  §14.1.5's "de-facto standard" description of Kubernetes: 84 percent of respondents were
...
- 📘 **Martin Fowler — "Continuous Integration"** —
  ...
  The canonical long-form article behind §14.2: mainline commits, self-testing builds,
...
- 📘 **Pete Hodgson — "Feature Toggles (aka Feature Flags)"** —
  ...
  The definitive long-form treatment behind §14.3.3: toggle categories (release, ops,
...
- 📘 **Martin Fowler — "DeploymentPipeline", "BlueGreenDeployment", and "CanaryRelease"**
  ...
  the deployment strategies of §14.3.2. *Access: free to read.*
- 📄 **Ross Harmes (Flickr) — "Flipping Out"** (2009) —
  ...
  a day, years before the tooling industry existed (§14.3.3). *Access: free to read.*
- 📄 **Chuck Rossi (Facebook Engineering) — "Rapid release at massive scale"** (2017) —
  ...
  code and features independently (§14.3.3). *Access: free to read.*
- 🛠 **OpenFeature** — [openfeature.dev](https://openfeature.dev/). The CNCF-incubating,
  vendor-neutral feature-flag API (§14.3.3); pairs with open-source flag backends such as
...
- 📄 **Minimum CD** — [minimumcd.org](https://minimumcd.org/). A community-maintained,
  ...
  — a useful checklist for auditing your own pipeline against §§14.2–14.3. *License: CC
  BY-SA 4.0.*
...
- 📘 **Google — Site Reliability Engineering (the SRE book and workbook)** —
  ...
  §14.3 and §14.7 at planetary scale. *License: CC BY-NC-ND 4.0 (free to read).*
...
- 🛠 **Docker — official documentation** — [docs.docker.com](https://docs.docker.com/).
  ...
  §14.4.1. *Access: free to read; Docker Engine is Apache-2.0.*
- 🛠 **Docker Compose documentation** —
  [docs.docker.com/compose](https://docs.docker.com/compose/). The Compose file reference
  and multi-service examples used in §14.4.2. *Access: free to read.*
- 🛠 **Docker Official Images — Postgres and Redis** —
  ...
  base images, with usage notes on volumes and environment configuration (§14.4.3).
...
- 📘 **The Twelve-Factor App** — [12factor.net](https://12factor.net/). The widely cited
  ...
  externalized-configuration rule of §14.4.4. *Access: free to read.*
- 📄 **Cloudflare Learning Center — DNS and reverse proxies** —
  ...
  Readable explainers for the DNS resolution chain (§14.5.1) and the edge-proxy model
  (§14.5.3). *Access: free to read.*
- 📄 **Let's Encrypt — How It Works, and ACME (RFC 8555)** —
  ...
  certificate authority and the protocol behind automatic HTTPS (§14.5.2). *Access: free;
...
- 🛠 **Caddy — automatic-HTTPS reverse proxy** —
  ...
  container (§14.5.2). *License: Apache-2.0.*
- 📄 **Google — HTTPS encryption on the web (Transparency Report)** —
  ...
  Ongoing measurement of the share of web traffic served over HTTPS (§14.5.2). *Access:
  free to read.*
...
- 📄 **OWASP — scanner-family references** —
  ...
  community definitions behind §14.6.1's three scanner families, with maintained tool
  lists. *Free to read; OWASP content is CC BY-SA.*
- 📄 **CISA — Software Bill of Materials (SBOM)** — [cisa.gov/sbom](https://www.cisa.gov/sbom).
  The U.S. government's SBOM resource hub referenced in §14.6.2. *License: U.S.
...
- 📄 **GitHub Docs — Dependabot** —
  ...
  Reference for the update-bot workflow of §14.6.2. *License: GitHub docs are CC BY 4.0.*
...
- 📄 **SLSA — Supply-chain Levels for Software Artifacts** — [slsa.dev](https://slsa.dev/).
  ...
  structured answer to the SolarWinds-style attacks of §14.6.2. *License: spec on GitHub
...
- 📄 **CISA — Alert AA20-352A** (December 2020) —
  ...
  government advisory on the SolarWinds compromise behind §14.6.2: malicious code inserted
...
- 📄 **npm — "kik, left-pad, and npm"** (March 2016) —
  ...
  registry's own account of §14.6.2's left-pad unpublication and the breakage that
...
- 📘 **Martin Fowler — "StranglerFigApplication"** —
  ...
  The original naming of §14.8.5's incremental-replacement pattern, with the fig-tree
...
- 📘 **Refactoring catalog** — [refactoring.com/catalog](https://refactoring.com/catalog/)
  ...
  treatment with examples). The named, small, behavior-preserving steps of §14.8.3.
...
- 📄 **Ward Cunningham — "The WyCash Portfolio Management System"** (OOPSLA 1992
  ...
  technical-debt metaphor (§14.8.4), worth reading to see how much the original meaning —
...
- 📄 **Robert L. Glass — "Frequently Forgotten Fundamental Facts about Software
  ...
  [doi.org](https://doi.org/10.1109/MS.2001.922739). Source for §14.8's maintenance-cost
...
- 📘 **Michael Feathers — *Working Effectively with Legacy Code*** (Prentice Hall, 2004) —
  ...
  The source of §14.8.1's tests-first definition of legacy code and the
