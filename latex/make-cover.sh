...
# Output: latex/swebook-generic-cover-wrap.pdf and latex/swebook-generic-cover-wrap.png
set -euo pipefail
cd "$(dirname "$0")/.."
...
# Keep a flattened PNG deliverable too (300dpi metadata, no alpha for print/preview use).
"$MAGICK" latex/cover-wrap.png -units PixelsPerInch -density 300 -background white -alpha remove -alpha off \
  latex/swebook-generic-cover-wrap.png
