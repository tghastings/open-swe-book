...
tex pandoc "$R/$B/combined.md" -f markdown-implicit_header_references -o "$R/$B/book.tex" -s \
  ...
  -V documentclass=scrbook -V classoption=11pt,twoside,openright,index=totoc 2>&1 | grep -iE 'error' || true

echo "=== xelatex + makeindex + xelatex x2 ==="
...
makeindex book >mi.log 2>&1 || echo "(makeindex issues)"
